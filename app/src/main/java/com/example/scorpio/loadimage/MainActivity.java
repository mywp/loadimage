package com.example.scorpio.loadimage;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void click(View view) {
        ...
        opts.inJustDecodeBounds =true;//不去解析真正的位图，只是获取这个位图的头文件信息
        ...
        int scale =1;
        if (dx>dy&&dy>1) {
            System.out.println("按照水平方向缩放，缩放比例："+dx);
            scale=dx;
        }

        if (dy > dx && dx > 1) {
            System.out.println("按照垂直方向缩放，缩放比例："+dy);
            scale=dy;
        }
        ...
        bitmap = BitmapFactory.decodeFile("/mnt/sdcard/photo.jpg");
        iv.setImageBitmap(bitmap);
    }
}
